...
export interface HttpLambdaFunctionConfig {
  assetPath: string;
  s3Bucket: S3Bucket;
  environment: string;
  lambdaName: string;
...
export class HttpLambdaFunction extends Construct {
  lambdaFunction: LambdaFunction;
  lambdaPermission: LambdaPermission;
  lambdaFunctionUrl: LambdaFunctionUrl;

  constructor(scope: Construct, id: string, config: HttpLambdaFunctionConfig) {
    super(scope, id);
    ...
    this.lambdaFunctionUrl = new LambdaFunctionUrl(this, "function_url", {
      functionName: this.lambdaFunction.functionName,
